...
from PLC_Model.TFM_modules import Embedder, PositionalEncoder, MultiHeadAttention, FeedForward, Norm
from collections import namedtuple
from PLC_Model.Vocab_word import Vocab
from PLC_Model.utils import create_src_mask, create_tgt_mask
...
class EncoderLayer(nn.Module):
    def __init__(self, d_model, heads, dropout = 0.1, pyramid = False):
        super().__init__()
        self.norm_1 = Norm(d_model)
        self.norm_2 = Norm(d_model)
        self.attn = MultiHeadAttention(heads, d_model, dropout)
        self.pyramid = pyramid
        self.ff = FeedForward(d_model, pyramid = self.pyramid)
    ...
    def forward(self, x, mask):
        ...
        if self.pyramid:
            idx_odd = torch.tensor([2*i+1 for i in range(x.shape[1]//2)], device = x.device)
            idx_even = idx_odd - 1
            x = (torch.index_select(x, 1, idx_odd) + torch.index_select(x, 1, idx_even))/2.0
        x = x + x2
...
class Encoder(nn.Module):
    def __init__(self, max_seq_len, vocab_size, d_model, N, heads, dropout = 0.1, pyramid = False):
        super().__init__()
        self.N = N
        self.pyramid = pyramid
        self.embed = Embedder(vocab_size, d_model)
        self.pe = PositionalEncoder(d_model, max_seq_len)
        self.layers = get_clones(EncoderLayer(d_model, heads, dropout, pyramid = self.pyramid), N)
    ...
    def forward(self, src, mask):
        ...
        if self.pyramid:
            for i in range(self.N):
                ...
                idx_odd = torch.tensor([2*i+1 for i in range(mask.shape[2]//2)], device = x.device)
                idx_even = idx_odd - 1
                mask_odd = torch.index_select(mask, 2, idx_odd)
                mask_even = torch.index_select(mask, 2, idx_even)
...
class Transformer(nn.Module):
    def __init__(self, max_enc_len, max_dec_len, vocab, d_model, enc_layers, dec_layers, heads, dropout, pyramid = False):
        super().__init__()
        ...
        self.pyramid = pyramid
        self.encoder = Encoder(self.max_enc_len, self.V_size, self.d_model, self.enc_layers, self.heads, self.dropout, pyramid = pyramid)
    ...
    def forward(self, src, tgt, src_mask, tgt_mask):
        if self.pyramid:
            e_outputs, src_mask = self.encoder(src, src_mask)
        else:
...
def test():
    ...
    src_batch = torch.empty(5,40).random_(10).long()
    tgt_batch = torch.empty(5,6).random_(10).long()
    pad = 0
    src_msk = create_src_mask(src_batch, pad)
    tgt_msk = create_tgt_mask(tgt_batch, pad)
    print(model(src_batch, tgt_batch, src_msk, tgt_msk))
